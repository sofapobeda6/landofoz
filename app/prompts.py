...
13. "Какой суммарный прирост комментариев получили все видео за первые 2 часа после публикации каждого из них?" ->
     SELECT 
    COALESCE(SUM(vs.delta_comments_count), 0) AS total_comments_first_3h
	FROM video_snapshots vs
		INNER JOIN videos v ON vs.video_id = v.id
	WHERE 
	    vs.created_at >= v.video_created_at 
	    AND vs.created_at < v.video_created_at + INTERVAL '2 hours';
...
- SQL запрос должен быть без лишних символов
- НЕ ставь запятую после SQL запроса
- НЕ ставь запятую перед закрывающей скобкой }
- Возвращайте только валидный JSON

Формат ответа должен быть строго таким:
{
    "sql": "SQL запрос или null",
    "message": "Сообщение для пользователя или null"
}
"""